...
class NonLinear():
    ...
    def _func_NLS(self, z, y):
        dy2 = np.gradient(np.gradient(y, self.dt), self.dt)
        dy3 = np.gradient(dy2, self.dt)
        dy4 = np.gradient(dy3, self.dt)
        return 1j / 2 * dy2 + 1j * abs(y)**2 * y \
               - self.eps3 * dy3 - 1j * self.eps4 * dy4
